...
impl NT7108 {
    pub fn new() -> io::Result<NT7108> {
        ...
        let ctrl_bits = (1<<E_BIT)|(DI_D<<DI_BIT)|(RW_R<<RW_BIT)|(RST_OFF<<RST_BIT)|(CS_DIS<<CS1_BIT)|(CS_DIS<<CS2_BIT)|(0<<BL_BIT);
    ...
    fn write(&mut self, chip: ChipId, is_data: bool, b: u8) -> io::Result<()> {
        self.update_ctrl_bits((1<<DI_BIT)|(1<<RW_BIT), ((if is_data {DI_D} else { DI_I })<<DI_BIT)|(RW_W<<RW_BIT))?;
    ...
    fn read(&mut self, chip: ChipId, is_data: bool) -> io::Result<u8> {
        ...
        self.update_ctrl_bits((1<<DI_BIT)|(1<<RW_BIT), ((if is_data { DI_D } else { DI_I })<<DI_BIT)|(RW_R<<RW_BIT))?;
        self.enable_chip(chip)?;
...
impl BufferedLcd {
    ...
    fn send_bytes(dev: &mut Lcd128x64, entries: &mut [BufferEntry], row: usize, col: usize) -> io::Result<()> {
        ...
        if n_written != entries.len() {
            return Err(io::Error::new(io::ErrorKind::Other, format!("Wrong number of bytes written ({} instead of {})", n_written, entries.len())));
        }
        for e in entries {
            e.dirty = false;
        }
